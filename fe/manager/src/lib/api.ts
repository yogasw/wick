...
export async function resyncMcpTools(key: string, id: string): Promise<{ ok: boolean; operations: number }> {
  return apiPost<{ ok: boolean; operations: number }>(`${rowBase(key, id)}/resync-tools`);
}
