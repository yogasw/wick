<script lang="ts">
  ...
  let fieldsByKey = $derived(new Map(fields.map((f) => [f.key, f])));

  function visible(field: ConfigField): boolean {
    return isFieldVisible(field, fieldsByKey, values);
  }
...
{:else}
  ...
  {#snippet kvSubBlock(field: ConfigField)}
    <div class="rounded-lg border border-white-300 dark:border-navy-600 overflow-hidden">
      ...
      <div class="p-4">
        <KvListField {field} onSave={(json) => onKvChange(field, json)} onCommit={() => onKvCommit(field)} />
      </div>
    </div>
  {/snippet}
        ...
        <div class="p-5 flex flex-col gap-4">
          {#if group.simple.length > 0}
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              {#each group.simple as field (field.key)}
                {#if visible(field)}
                  {@render simpleFieldBlock(field)}
                {/if}
              {/each}
            </div>
          {/if}
          {#each group.kvlists as field (field.key)}
            {#if visible(field)}
              {@render kvSubBlock(field)}
            {/if}
          {/each}
        </div>
      </details>
