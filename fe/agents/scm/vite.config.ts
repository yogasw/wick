...
// fe/agents/scm → up 3 → repo root.
const REPO_ROOT = path.resolve(__dirname, "../../..");
const OUT_DIR = path.resolve(REPO_ROOT, "internal/tools/agents/dist/scm");
...
export default defineConfig({
  plugins: [svelte()],
  base: "/tools/agents/workflow/scm/",
  build: {
    outDir: OUT_DIR,
    emptyOutDir: !WATCH,
    assetsDir: "assets",
  ...
  server: {
    ...
    proxy: {
      "/tools/agents": {
        ...
        changeOrigin: true,
        bypass: (req) => {
          if (req.url?.startsWith("/tools/agents/workflow/scm/")) {
            return req.url;
          }
        },
      },
