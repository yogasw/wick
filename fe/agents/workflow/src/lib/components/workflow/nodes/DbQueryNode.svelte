...
>
  {#snippet body()}
    <div class="text-[10px] text-black-500 dark:text-white-100-700">{node.database ?? "default"}</div>
    <pre class="font-mono text-[10px] line-clamp-2 whitespace-pre-wrap">{node.sql ?? "—"}</pre>
  {/snippet}
</BaseNode>
