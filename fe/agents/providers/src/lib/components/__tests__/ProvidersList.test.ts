...
describe("ProvidersList - wick built-in card", () => {
  ...
  it("renders the Built-in badge + model count and calls onNavigate", async () => {
    ...
    vi.mocked(api.apiGetWickConfig).mockResolvedValue({
      models: [
        { ID: "m_1", Kind: "google", Label: "Gemini Flash", Model: "gemini-flash-latest", KeyMasked: "••", HasKey: true, BaseURL: "", APIFormat: "gemini", MaxOutputTokens: 0, Default: true, Disabled: false, Temperature: null, TopP: null, ThinkingBudget: null, RawConfig: "", LiveSet: false, DiscoveryFilter: "", DefaultVendorModel: "" },
      ],
      settings: { ShellToolDisabled: false, ShowCapabilities: true, CapabilityMode: "icon", Connectors: [], MaxContextTokens: 0, MaxTurns: 0, MaxConsecErrors: 0, MaxTurnMinutes: 0, MaxModelRetries: 0, ModelCallTimeoutSec: 0, Temperature: null, TopP: null, ThinkingBudget: null, RawConfig: "" },
    });
    const onNavigate = vi.fn();
