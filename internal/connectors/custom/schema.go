...
// DefOp is one operation of a custom connector — both the persisted
// shape (entity.CustomConnector.Ops is a JSON array of DefOp; array
// order is display order) and the parser/review-form interchange shape
// shared by the cURL parser, the AI parser, the MCP import, and the
// manual builder. Exactly one of Request / MCPSource is set.
type DefOp struct {
	Key         string     `json:"key"`
...
// ParseOps decodes the entity.CustomConnector.Ops JSON column. Empty
// input yields an empty slice.
func ParseOps(raw string) ([]DefOp, error) {
	if strings.TrimSpace(raw) == "" {
		return []DefOp{}, nil
	}
	var out []DefOp
...
// entity.CustomConnector row; Category lands in SourceMeta. Single
// locks the def to one instance row — default off (multi-row, same as
// built-ins).
type Draft struct {
	Key         string     `json:"key"`
	Name        string     `json:"name"`
...
func ValidateDraft(d *Draft) error {
	...
	for i, op := range d.Ops {
		if !keyRe.MatchString(op.Key) {
			return fmt.Errorf("op %d: key %q must be snake_case", i, op.Key)
		}
		if seen[op.Key] {
