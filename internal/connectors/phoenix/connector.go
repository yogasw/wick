...
func Operations() []connector.Operation {
	return []connector.Operation{
		...
		connector.Op(
			"get_span",
			"Get Span Detail",
			"Fetch the full detail of one span by its span_node_id: every message (system prompt, user turns, tool messages), the model output, any tool_calls the model requested, token usage, latency, and cost. This is the core 'why did the AI answer X' view.",
			GetSpanInput{},
			getSpan,
			wickdocs.Docs{
				OutputShape: map[string]string{
					...
					"tokens":   "Object with prompt, completion, total counts.",
				...
				OutputSample: `{"span_node_id":"U3BhbjozODk2MjI0MQ==","name":"ChatCompletion","model":"gpt-4o","messages":[{"role":"system","content":"You are..."},{"role":"user","content":"refund gagal"}],"output":"Maaf...","tokens":{"prompt":1100,"completion":134,"total":1234},"latency_ms":820}`,
			},
		),
	}
}
