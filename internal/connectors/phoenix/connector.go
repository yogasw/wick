...
// global id (e.g. "YWJjZA==" = "abcd"), copied from the project
...
type Configs struct {
	...
	ProjectID string `wick:"required;desc=Phoenix project global id, base64 form. Example: YWJjZA== (decodes to abcd). Copy it from the project URL or the GraphQL API."`
}

// ListSpansByRoomInput drives list_spans_by_room.
type ListSpansByRoomInput struct {
	RoomID    string `wick:"required;desc=Conversation room id. Matched against Phoenix session id. Example: 123456789"`
...
type ListSpansByAppInput struct {
	AppID     string `wick:"required;desc=Application id stored in span metadata['app_id']. Example: myapp-a1b2c3d4"`
...
func Operations() []connector.Category {
	return []connector.Category{
		connector.Cat(
			"Phoenix",
			"Inspect LLM spans and traces captured by Arize Phoenix.",
			connector.Op(
				"list_spans_by_room",
				"List Spans by Room",
				"List LLM spans for a conversation room (Phoenix session). Returns a summary per span — model, status, tokens, latency, plus previews of the system prompt, last user input, and output. Use this first to find the span to inspect, then call get_span with its span_node_id. Empty result = no spans in range.",
				...
				wickdocs.Docs{
					...
					InputSample:  `{"room_id":"123456789","llm_only":true}`,
					OutputSample: `{"room_id":"123456789","count":1,"spans":[{"span_node_id":"U3BhbjozODk2MjI0MQ==","name":"ChatCompletion","model":"gpt-4o","status":"OK","tokens_total":1234,"latency_ms":820,"system_prompt_preview":"You are a helpful...","input_preview":"refund saya gagal","output_preview":"Maaf atas kendalanya..."}]}`,
				},
			),
			connector.Op(
				"list_spans_by_app",
				...
				wickdocs.Docs{
					...
					PairWith:     []string{"connector:phoenix.get_span"},
					InputSample:  `{"app_id":"myapp-a1b2c3d4","max_spans":100,"root_only":true}`,
					OutputSample: `{"app_id":"myapp-a1b2c3d4","count":1,"spans":[{"span_node_id":"U3Bhbjo0MA==","name":"agent","span_kind":"agent","status":"OK","trace_id":"abc123","input_preview":"...","output_preview":"..."}]}`,
				},
			),
			connector.Op(
				"get_span",
