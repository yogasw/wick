...
// proxied to 9router and never captured at all (see hasSubscribers).
//
// Each subscriber is the Requests tab of an open 9router page, connected
// over Server-Sent Events. Closing the tab removes the subscriber; the
// full request/response bodies live only in that browser's memory for as
...
// carries the FULL request and response bodies (never truncated) — they
// are held only in the receiving browser, so size is the client's concern,
// not the server's. Field names are the JSON keys the UI reads.
type ReqEvent struct {
	Time       string `json:"time"`        // wall-clock "15:04:05" of when the request landed
	Method     string `json:"method"`      // HTTP method
	Path       string `json:"path"`        // request path (already stripped of the /9router prefix)
...
// subChanBuffer bounds how many events a slow subscriber may queue before
// we drop new ones for it. A watching browser reads promptly; this only
// guards against a stalled client wedging the publisher.
...
// proxy consults this BEFORE capturing bodies: with no watcher, the
// request is proxied untouched and nothing is buffered. Lock-free.
func (b *broadcaster) hasSubscribers() bool { return b.count.Load() > 0 }

// publish delivers e to every current subscriber, non-blocking. A
// subscriber whose buffer is full misses this event rather than stalling
// the publisher (the browser will still get subsequent events).
...
func redactAuth(raw string) string {
	...
	// Drop a leading scheme ("Bearer ", "Basic ", ...).
...
// spots 9router honours (Authorization: Bearer, x-api-key, x-goog-api-key)
...
// sniffModel best-effort extracts the "model" field from a JSON request
// body without a full parse — cheap enough to run on every request. It
// looks for the first `"model"` key and reads the quoted string value.
// Returns "" when not found (e.g. non-JSON or a form we don't recognise).
...
// captureWriter wraps a ResponseWriter to record the upstream status and
// a FULL copy of the response body while streaming it through to the
// client untouched. It flushes on every Write so SSE token streams still
// reach the caller in real time. Only used when a subscriber is watching.
...
// isLoopbackHost reports whether hostport (a Host header or RemoteAddr,
// with or without a port) refers to the loopback interface. Local copy so
// this package doesn't depend on the api package.
