...
type GeneralConfig struct {
	...
	PreemptIdle               bool   `wick:"bool;desc=When the pool is full and a new session is queued, preempt the longest-idle active subprocess to free its slot. Killed sessions resume via --resume on their next message."`
	...
	WorkflowMaxParallelGlobal int    `wick:"number;desc=Global parallel cap. 0 = parallel disabled, all workflows serial (default). N > 0 = parallel enabled; at most N runs execute simultaneously across all workflows. Per-workflow concurrency.max is honoured as an inner cap."`
