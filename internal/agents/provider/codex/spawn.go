...
func (s Spawner) Spawn(ctx context.Context, opt provider.SpawnOptions) (provider.Process, error) {
	...
	// instead of the user hand-typing it in extra_args. Kept in the router's
	// SpawnHook so it stays unit-testable without spawning a real binary.
	routerContrib, err := provider.RouterSpawnContribution(opt.Instance, provider.TypeCodex)
	if err != nil {
		return nil, err
	}
