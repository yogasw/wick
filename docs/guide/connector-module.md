...
6. **Access Policy** — five flags controlling who can configure, connect, or see OAuth accounts. Editable by admins or the instance owner (creator); `AllowOthersConfigure` users are intentionally excluded here because the policy controls who gets that grant:
   ...
   - `Others can see connected accounts` — off by default: every connected account is **private to the wick user who connected it**, unless an admin shares that one account with a tag (`/admin/connectors` lists each instance's connected accounts underneath it, each with its own tag picker; tags live in `tool_tags` on `/connector-accounts/{accountID}`). Each user's `wick_list` shows the connector plus their own account, the detail page lists only their own, and running an op as an account somebody else connected is refused. Admins and the instance owner always see the whole pool. Turn it on to go back to a shared pool where anyone with tag access can see and run as every account. Stored on `entity.Connector.AllowOthersSeeAccounts`; the rule lives in `connectors.AccountVisibleTo`.
