...
| **Workflows** — YAML DAG of classify / agent / connector / http / channel / dataset / branch / parallel nodes triggered by cron, channel events, webhooks, or manual runs; replayable per-run state on disk; visual canvas + MCP `workflow_*` ops | [Workflows](./agents/workflow) |
