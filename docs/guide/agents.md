...
| `SenderVisibility` (group **Session Identity**) | `name` | How much of the sender's identity is written into each message the agent receives — `off` (nothing), `name` (display name only), `name_id` (adds the platform user ID, for @-mentioning/DMing a specific person), or `full` (adds channel + handle too). Governs only the model's copy of the message; the dashboard always shows the full sender regardless. See [Channels ▶ Sender identity](./agents/channels#sender-identity). |
