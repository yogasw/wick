...
**Keeping it in sync:** the operation set mirrors the server's live `tools/list` at every module build (boot, save, re-sync) — the catalog is never cached. To pick up tools added or removed upstream you have two options:

- **Definition-level re-sync** — click **↻ Re-sync tools** on the connector definition page. Available to admins and the definition's creator.
- **Per-instance re-sync** — on an individual instance's detail page (Manager → Connectors → {instance}), the **Credentials** section shows a **Re-sync tools** button for custom MCP instances when the caller can mutate the definition (admin or creator). This re-fetches `tools/list` under that instance's connected account (relevant for `oauth` schemes) and refreshes the stored connection status.
...
**Status (MCP definitions):** both the connector definition page and each instance's detail page header show a connection chip — **● Connected**, **● Disconnected**, or **● Never tested** — for custom MCP connectors. The status refreshes on every module rebuild (boot, **↻ Re-sync tools**, server-form save), so it stays Connected until a re-sync, reconnect, or disable says otherwise. cURL/manual definitions have no connection to track and show no chip.
