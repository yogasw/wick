...
**Tool grouping via `_meta`:** When the server's `tools/list` response carries a top-level `_meta.categories` legend (an array of `{title, description}` objects) and individual tools set a `_meta.category` field matching a legend title, wick groups the exposed operations into titled sections — the same sections visible on the connector detail page and returned by `wick_get` level 1. Section order follows the server's legend. Tools with no `_meta.category` collect into one untitled trailing section. Servers that ship no `_meta` at all keep the historical flat (untitled, single-section) layout — nothing changes for them.
