...
### Added
*   **Connected accounts — inline operations editor** — on an OAuth connector's detail page the **Connected accounts** section now has a **Manage operations** button per account (visible to admins and the account's own user). Clicking it expands an inline checkbox list of the connector's operations; unchecking an operation disables it for that account only, leaving other accounts and the shared connector-level toggle untouched. Click **Save** to persist the change. The read-only "N operation(s) disabled for this account" count is retained as a summary when the editor is closed.
