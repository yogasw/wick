...
-   **Memory hardening — provider storage**: Listing, explorer, retention, and purge queries no longer load file content blobs into memory, fixing an ~800 MiB memory peak. A new `size` column is persisted on write and backfilled on migration, allowing the file-list endpoint to report sizes without fetching content. Folder-zip downloads now fetch content blobs individually by ID.
-   **Bounded HTTP reads**: The `http` workflow node and the generic internal HTTP client now cap response buffering at **64 MiB**, returning an error for larger responses. Previously, these were unbounded `io.ReadAll` calls.
-   **Webhook body cap**: Inbound webhook trigger requests are now rejected with a `413` status if the body exceeds **10 MiB**.
-   **Access-log middleware**: Large or streaming request bodies (`multipart/form-data`, `application/octet-stream`, `text/event-stream`, or `Content-Length > 64 KiB`) are no longer buffered by the logger, allowing the downstream handler to read them directly.
-   **Opt-in pprof** (`WICK_PPROF=1`): Set this environment variable to expose Go pprof endpoints on `127.0.0.1:6060` for heap and CPU profiling. This feature is loopback-only and never exposed on the public listener.
