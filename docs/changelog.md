...
### Added
*   **`update_page_properties` for `notion_unofficial`**: Edits the property cells of an existing database row in place (status, date, select, multi_select, checkbox, relation, person, …) — only the properties passed are written, the rest of the row and its body content are untouched. Takes `page_id` (the row) and `properties` (JSON `name → value`, same shapes as `create_page`); refuses a plain page (no property schema — use `set_title`/`update_block` there), and unknown/read-only names come back in `skipped_properties` instead of failing. Returns `{id, updated, skipped_properties}`. Call `describe_database` first for exact property names/types/options. See [Notion (Unofficial) ▶ Writing database row properties](/connectors/notion_unofficial#writing-database-row-properties).
*   **`notion_unofficial` requires an AI description to be set up**: Since this connector authenticates with a personal Notion session token (`token_v2`) — where every call acts as one human across their whole workspace — an instance is now considered `needs_setup` until its per-instance **AI description** is filled, so there is always a record of who may use it and what for. This reuses the existing per-instance description field (the guidance the LLM reads) and the standard readiness status: a blank description reports as unfinished in the manager UI, exactly like a missing required config field, and flips to `ready` once written. Setup widgets (`Import`, `Status`) work regardless so an operator can confirm the connection first. Backed by a new opt-in `Meta.RequireAIDescription` connector flag. All auth/advanced config fields (`token_v2`, `active_user_id`, `user_agent`, `notion_client_version`) are now required too — filled in one click by the Extract widget. Connector bumped to 0.4.0. See [Notion (Unofficial) ▶ AI description required](/connectors/notion_unofficial#ai-description-required).
*   **`operator_note` in the connector meta-tools**: `wick_list`, `wick_search`, and `wick_get` now include a separate `operator_note` field carrying the admin-written per-instance AI description, distinct from the connector's built-in `description`. This lets the agent tell an operator-authored instruction (access rules, team notes, constraints) from the product description and give it extra weight — e.g. a personal-token connector whose note says who may use it. Omitted when blank.
