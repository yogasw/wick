...
## [v0.9.0](https://github.com/yogasw/wick/compare/v0.8.11...v0.9.0)

_Released on 2026-05-10_

### Added
- Agent system foundation including filesystem-backed state, subprocess management, and a per-agent state machine.
- Command security gate via the `wick-gate` binary, allowing for glob-based whitelisting of shell commands.
- `wick doctor` diagnostic command to check local environment requirements and MCP client registrations.
...
- Prometheus-compatible `/metrics` endpoint for tracking connector execution telemetry and latency.
- Per-operation access control allowing specific connector actions to be restricted to administrators.
- Sliding-window rate limiting for connectors to protect external APIs from excessive calls.
- Comprehensive audit log API and Admin UI for monitoring cross-connector run history and status.
- `pkg/conntest` helper package to simplify unit testing for custom connector authors.

### Fixed
- Improved updater reliability on Windows and Linux by verifying post-install state and using detached helper scripts to prevent partial installs.
- Resolved race conditions in the agent pool manager that caused file rename collisions on Windows.
- Fixed missing request logs in lab and CLI modes by correctly injecting component-tagged loggers into the execution context.
- Corrected server banner to display the actual configured application URL and log host mismatches during 403 rejections.

### Improved
- Aligned the Claude agent parser and spawner with the real version 2.1.x stream-json protocol and long-lived process lifecycle.
- Refactored internal command structure by renaming `cmd/lab` to `cmd/wick-lab`.
- Enhanced agent testing infrastructure with multi-turn and multi-session integration scenarios using simulated spawners.
- Synchronized design documentation to match implementation details for agent phases 1 through 3.
