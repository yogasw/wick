...
## [v0.4.0](https://github.com/yogasw/wick/compare/v0.3.0...v0.4.0)

_Released on 2026-05-01_

### Added
- New `/mcp` endpoint supporting Personal Access Tokens (PAT) and OAuth 2.1 bearer authentication.
- Model Context Protocol (MCP) Streamable HTTP support (2025-03-26) for progress notifications and heartbeats via Server-Sent Events (SSE).
- Meta-tool pattern for MCP (`wick_list`, `wick_search`, `wick_get`, and `wick_execute`) to optimize tool discovery and minimize token usage.
- Connector management UI (`/manager/connectors`) featuring instance cards, credential editors, and operation-level toggles.
- Standalone connector test panel with Postman-style execution, status/latency metrics, and JSON response previews.
- Dedicated connector history page with URL-driven filters for operations, source, status, and users.
- Admin overview pages for global management of connectors, access tokens, and OAuth 2.1 grants.
- "System" tag functionality to protect core maintenance entities and jobs from manual deletion or modification.
- Automated `connector_runs` purge job that daily cleans up audit logs older than the retention period.
- History retry functionality that prefills the test panel with parameters from prior connector runs.
- Sample connector implementation for CRUD operations.

### Improved
- Optimized MCP schema delivery by stripping `input_schema` from list/search results and moving them to a dedicated `wick_get` tool.
- Refactored LLM tool discovery to use stable meta-tools, eliminating the need for manual tool list refreshes when adding connectors.
- Split admin dashboard navigation into "Modules" and "Access" dropdowns for better categorization.
- Paginated connector history view with 10 entries per page and navigation controls.
- Enhanced job registration to ensure maintenance tasks are visible across both web and worker processes.
- Simplified connector detail surface by moving primary management actions to the list-view kebab menu.
- Strengthened `Ctx.HTTP` implementation to prevent goroutine leaks during upstream hangs.

### Fixed
- Resolved an issue causing failed time parsing in specific database interactions.
- Fixed a bug where action connectors failed to auto-hide correctly in the UI.
- Corrected a flaw where "System" tags created before schema updates were not properly flagged as immutable.
- Fixed an issue where system-managed jobs could still be modified or untagged via the admin picker.
