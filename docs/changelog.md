...
*   **Tickets as entities, with notes and a project-settings revamp:**
    ...
    *   Board columns are per project: name your own stages (`triage → coding → shipped`) with one marked as the finished stage, which is where auto-resolve moves completed work. Removing a column that still holds tickets is refused rather than stranding them.
