...
*   **A result panel for list buttons, not a toast**: A board action now renders the receiver's whole JSON reply — status, message, a `progress` bar, `counts` chips, and its own `html` in the same sandboxed frame an HTML artifact gets (theme variables included). While the reply says the job is running, wick polls the `poll_url` it handed back (through `POST /api/projects/{id}/board-actions/{buttonID}/poll`, which only fetches URLs on the button's own origin) and **re-reads the board every time the counters move**, so cards move to their new column as the job walks the list. A ticket-page button now re-reads its ticket after a successful click for the same reason.
