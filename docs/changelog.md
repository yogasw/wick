...
*   **A result panel for list buttons, not a toast**: A board action now renders the receiver's whole JSON reply — status, message, a `progress` bar and `counts` chips — as a small floating card in the bottom-right corner that closes itself 8 seconds after a run ends (a failure stays). A receiver's own `html` is folded behind a Details toggle and rendered in a bare sandboxed frame, so a status line can never take a third of the board. While the reply says the job is running, wick polls the `poll_url` it handed back (through `POST /api/projects/{id}/board-actions/{buttonID}/poll`, which only fetches URLs on the button's own origin) and **re-reads the board every time the counters move**, so cards move to their new column as the job walks the list. A ticket-page button now re-reads its ticket after a successful click for the same reason.
*   **Button events carry the person's email**: `actor.email`, and `board.assignee_email` on a list button. A receiver has to recognise these people in its own system, where a wick user id means nothing and a name is ambiguous — matching on the identifier both systems already share beats a hand-maintained id table.
*   **CLI channel — long work reports back**: an agent can mint a short-lived, session-bound token (`wick_cli_token`) and hand it to a build, deploy or migration; the job ends with `support-tools agent send --text "…"` and the session wakes on that message instead of polling for it. The token is bound to the session that minted it (the endpoints take no session id at all), expires in 30 minutes by default and 2 hours at most, lives in memory, and reaches nothing but `/api/cli/send` and `/api/cli/whoami`. There is deliberately no CLI command that mints one — a session id is not a secret, so that would be a way to speak into anybody's conversation. Exit codes distinguish an expired token (3) from an unreachable daemon (4) from a refusal (5), so a script can branch on what actually went wrong. See the new [CLI channel](/guide/agents/cli-channel) guide and the shipped `wick-cli-channel` skill.
*   **The idle timers read `touched_at`, not `updated_at`**: a mirrored ticket wears the source system's edit date, so the auto-resolve sweeper was closing imported tickets on arrival — 130 of them in one run on a board with a two-day auto-resolve. Every write now also stamps `touched_at` (when wick wrote it), and the follow-up and auto-resolve windows measure from that; tickets written before the field existed fall back to `updated_at`.
*   **`GET /projects/{id}/tickets?fields=all`**: the board sends only the fields a project marked *show on card*, which is right for a board and wrong for a machine — a sync's own join key (`notion_page_id` and friends) is exactly what nobody marks, so it could not recognise the tickets it had created and tried to create them again every run.
*   **`updated_at` can come from the system being mirrored**: `PATCH /tickets/{id}` now accepts `updated_at` — an RFC3339 timestamp (the source system's own edit time) or `"keep"` (a bookkeeping write that must not move the clock). Without it a sync stamped `now` on every ticket it touched, so a run over a hundred pages made the whole board read "just now" and sorted it by when the importer ran.
*   **The git watcher paces itself by what a scan costs**: The Source panel's watcher ran a full repo walk on a fixed 400ms debounce, so anything writing files continuously (a build, an agent in a loop, a log appended twice a second) kept it firing — on a session directory holding 61 clones that was 150–180% CPU in the daemon itself, on a 2-core host, while the build sat inside its own cgroup quota. It now waits 4× the duration of the last pass (capped at 15s) before running another, holding it to roughly a fifth of a core however large the directory is. Small trees are unaffected.
