...
*   **Slack auto-register** (`AutoRegister`, default off): creates a wick account for a Slack sender whose email has none. The account arrives **unapproved**, never admin, and with no password — Slack vouches that the address is on the workspace, not that the sender controls it, so an admin approving it under **Admin → Users** is what turns one claim into the other.
...
*   **Approval notices reach admins too:** when a user is approved, the other approved admins are notified alongside the user. Approval is the moment an account gains access, so this is the audit trail — without it only the admin who clicked knows it happened.
