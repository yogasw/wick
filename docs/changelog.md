...
### Added
*   **Custom MCP connector — per-instance Re-sync tools** — a **Re-sync tools** button is now shown in the Credentials section of a custom MCP connector's instance detail page (Manager → Connectors → {instance}). Clicking it re-fetches the upstream server's `tools/list` under that instance's connected account (honoring `oauth` token scopes) and atomically swaps in the fresh operation set, refreshing the stored connection status. Visible only to admins and the definition's creator. A definition-level re-sync (connector page) remains available alongside this.
*   **Custom MCP connector — connection status chip on instance detail** — the instance detail page header now shows a **Connected / Disconnected / Never tested** chip for custom MCP connectors, mirroring the chip already present on the definition page.
