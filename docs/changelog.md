...
### Added
- **Workflows** — multi-step YAML DAG automations under `<BaseDir>/workflows/<id>/`, with typed nodes (`classify`, `agent`, `connector`, `channel`, `http`, `shell`, `db_query`, `transform`, `go_script`, `branch`, `switch`, `parallel`, `merge`, `dataset_*`, `session_init`, `end`) and triggers (`cron`, `channel`, `webhook`, `manual`, `schedule_at`, `error`). See [Workflows guide](/guide/agents/workflow).
