...
*   **Zero-downtime binary replacement (`<app> reload`)**: A `restart` closed the listening port for the successor's entire boot — registry restore, database, connectors, tens of seconds — and killed every in-flight agent turn, workflow run and cron job. `reload` instead starts a successor, hands it the **listening socket**, and keeps serving until that successor is ready; nothing in front of wick sees a refused connection. The old process then drains and exits. What it waits for is split by what interrupting it costs: work that cannot be resumed (a workflow run stopped mid-node, a cron job mid-write, a connector call, a plugin request, a scheduled delivery) gets `WICK_DRAIN_TIMEOUT`, default 20m; agent turns get a short `WICK_DRAIN_AGENT_GRACE`, default 45s, because the session is on disk and resumes on the next message. An intake baton (a lock file in the data dir) keeps exactly one process accepting new work during the overlap, so Slack socket-mode events are never split between two binaries and cron never fires twice. Opt-in with `WICK_GRACEFUL_UPGRADE=1`; full support on Linux/macOS/BSD as a plain daemon or under systemd (`Type=notify` + `NotifyAccess=all` + `ExecReload`), while Windows and the tray's in-process supervisor fall back to the old stop/start. New background subsystems declare their own work with `upgrade.Register` / `upgrade.RegisterResumable` so a drain never carries a hard-coded list. See [`<app> reload`](/reference/app-cli#app-reload) and [Headless ▶ Updating without downtime](/guide/headless#updating-without-downtime).
*   **Slack: run an agent inside a thread from a workflow (`slack.send_to_session`)**: A workflow's `agent` node runs headless `claude --print` — no MCP, skills unavailable, and none of its reasoning visible, which makes a multi-minute run a black box in a channel where people are waiting. The new action injects the task into the agent session bound to a Slack thread, exactly as if a person had typed it there, so the turn is an ordinary pool turn: status banner, tool activity streaming into the loading bubble, reply in the thread, full toolset. Fire-and-forget — it returns when the pool accepts the message, so nothing downstream can read the answer. `slack.send_message` also gains `auto_reply`, which arms the thread's 🤖 switch after posting (a bot cannot arm it by reacting, and a bot-posted app-mention never reaches a trigger), and bot messages now reach the workflow surface as `message` / `thread_started` with `is_bot`, `bot_id` and `bot_username`, guarded so an instance never sees its own posts.
*   **Self-update now uses the handover too**: Applying a staged update swapped the binary and `syscall.Exec`'d in place, which replaces the process image — the listening socket closed with it and every in-flight turn died. When graceful upgrade is armed, the swap is followed by a socket handover instead: the binary is replaced, a successor is forked from it, and this process keeps serving until the successor is ready before draining. The tray keeps the classic swap + re-exec, since it runs the server in-process and cannot hand a socket to another process.
*   **Built-in skill: upgrading without downtime**: Ships in the binary and extracts into wick's own skills dir — reload vs restart, the systemd lines a handover needs, why the binary must be renamed rather than copied over, what the drain waits for, how to register a new background subsystem, how to prove from outside the host that nothing was dropped, and the failure modes worth recognising.

### Fixed

*   **Source panel missed changes deeper than one directory**: `fsnotify` is not recursive, and the watcher registered only the session cwd, each repo root and one level below it — so an edit to `app/app.go` was noticed while an edit to `internal/pkg/api/server.go` was not, and the panel stayed stale until the page was reloaded. The active repo's tree is now watched in full (skipping `.git` and build/cache noise), other repos stay shallow, and the walk is capped at 4000 directories because inotify watches are a shared per-user kernel resource and a session cwd can hold dozens of clones. Each repo's `.git` is watched too, so staging or committing from a terminal refreshes the panel.
*   **Source rail badge showed no counter until a file changed**: The badge is rendered only from `git_status` events, and the SharedWorker keeps one stream alive across page loads — so a reloaded page never re-opened the stream and never received a snapshot. The last `git_status` per session is now replayed in the snapshot the worker requests on every page load, and a subscriber joining an existing watcher gets a fresh publish.
*   **Assistant message duplicated on every page reload**: The stream snapshot's disk fallback replayed assistant text as `text_delta`, which means *append* — and that path runs on every page load, so each reload appended the same text again. It now emits one cumulative `text_snapshot`, the fix the pool path already had.
*   **A reply typed in the web UI never reached its Slack thread**: The session → (channel, thread) mapping lived only in memory and was filled only by the path that receives a Slack message, so after a restart a reply typed in the web UI of a Slack-originated session — and any session created by a workflow — produced an answer that was written to the session and posted nowhere. The binding is now persisted on the session (`meta.channel_ref`) the same way `auto_reply` already is, and the Slack channel restores a turn from it when it has none. A session with no binding is never answered into a Slack channel, and a session belonging to another instance is left alone.
*   **`plugin.Reloader.Stop` panicked on overlapping shutdown paths**: The stop channel was closed twice (`close of closed channel`). It is idempotent now.
*   **Workflow subsystem was never stopped on shutdown**: `wfsetup.Manager.Stop` had no caller in non-test code, so a restart cut running workflows off mid-node.
