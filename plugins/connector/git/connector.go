...
type Config struct {
	...
	Token      string `wick:"secret;group=Authentication;desc=Personal access token, App Password, or API token. Passed to git through an askpass helper and never written to disk. Minimum scopes — GitHub classic PAT: repo (private) or public_repo (public only). GitHub fine-grained PAT: Repository permissions, Contents Read and write. Bitbucket Cloud App Password: Repositories Read plus Write. Bitbucket Server HTTP access token: Project read and Repository read, plus Repository write to push. GitLab: read_repository, plus write_repository to push. Read-only operations need only the read scope."`
